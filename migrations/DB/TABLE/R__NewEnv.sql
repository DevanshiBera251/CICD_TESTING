...
SET DATABASE_HORMONIZED_NAME = CONCAT('DB',$env, '_HARMONIZED1');
SET DATABASE_CONSUMPTION_NAME = CONCAT('DB',$env,'_CONSUMPTION1');
set RESOURCE_NOTIFY_USER = 'DEVANSHITRIAL'; 
--set USER_NAME1 = 'DEVANSHI_BERA';
--set RESOURCE_NOTIFY_USER = 'SWETHA'; 
--set USER_NAME1 = 'DEVANSHI_BERA';
-- set USER_NAME2 = 'SWETHAY';
--  set USER_NAME3 = 'RAHULKUMARJAIN';
-- -- set USER_NAME4 = '';
...
set INGESTION_WAREHOUSE = concat('WH',$env,'_INGESTION_XS');   --WH_DEV_INGESTION_XS --DATALOADING_WAREHOUSE
set ENGINEER_WAREHOUSE = concat('WH',$env,'_ENGINEER_XS');
set CONSUMPTION_WAREHOUSE = concat('WH',$env,'_REPORTER_XS'); --CONSUMPTION
SET MASKING_WAREHOUSE = concat('WH',$env,'_MASKING_XS');

set ADMIN_WAREHOUSE = concat('WH',$env,'_ADMIN_XS');

--RESOURCE MONITOR PARAMETER NAMES 
--SET RM_PARAM_DEV='RMON_WH_DEV'
SET DATALODING_WH_MONITOR = CONCAT('RMON_','WH',$env,'_INGESTION'); --ADMIN_WH_MONITOR
SET ENGINEER_WH_MONITOR = CONCAT('RMON_','WH',$env,'_ENGINEER');
SET CONSUMPTION_WH_MONITOR = CONCAT('RMON_','WH',$env,'_CONSUMPTION');
SET ADMIN_WH_MONITOR = CONCAT('RMON_','WH',$env,'_ADMIN');

--ACCESS ANF FUNCTIONAL ROLES NAMES
--SET DEV_AR_PARAM = 'ROLE_AR_DEV_SCH'
SET HARMONIZED_SRW = CONCAT('ROLE_AR',$env,'_SCH_COMMON_DATA_MODEL_SRW'); --HARMONIZED_RW
SET HARMONIZED_SRO = CONCAT('ROLE_AR',$env,'_SCH_COMMON_DATA_MODEL_SRO'); --HARMONIZED_RO
SET HARMONIZED_SFULL = CONCAT('ROLE_AR',$env,'_SCH_COMMON_DATA_MODEL_SFULL'); --HARMONIZED_ARW

SET CONSUMPTION_SRW = CONCAT('ROLE_AR',$env,'_SCH_REPORTING_SRW'); --CONSUMPTION_RW
SET CONSUMPTION_SRO = CONCAT('ROLE_AR',$env,'_SCH_REPORTING_SRO'); --CONSUMPTION_RO
SET CONSUMPTION_SFULL = CONCAT('ROLE_AR',$env,'_SCH_REPORTING_SFULL'); --CONSUMPTION_ARW

SET RAW_FAST_SRW = CONCAT('ROLE_AR',$env,'_SCH_FAST_SRW'); --RAW_FAST_RW
SET RAW_FAST_SRO = CONCAT('ROLE_AR',$env,'_SCH_FAST_SRO'); --RAW_FAST_RO
SET RAW_FAST_SFULL = CONCAT('ROLE_AR',$env,'_SCH_FAST_SFULL'); --RAW_FAST_ARW

SET RAW_ILIFE_SRW = CONCAT('ROLE_AR',$env,'_SCH_ILIFE_SRW'); --UTILS_RO REPLACE THIS with new parameter below this  
                                                          --RAW_ILIFE_RW 
SET RAW_ILIFE_SRO = CONCAT('ROLE_AR',$env,'_SCH_ILIFE_SRO'); --UTILS_RO
SET RAW_ILIFE_SFULL = CONCAT('ROLE_AR',$env,'_SCH_ILIFE_SFULL'); --UTILS_ARW

--FUNCTIONAL ROLE NAMES CREATION
--SET DEV_FR_PARAM = 'ROLE_FR_DEV'
SET INGESTION = CONCAT('ROLE_FR',$env,'_INGESTION');
SET ENGINEER = CONCAT('ROLE_FR',$env,'_ENGINEER');
SET REPORTER = CONCAT('ROLE_FR',$env,'_REPORTER');
SET MASKING = CONCAT('ROLE_FR',$env,'_MASKING');

--SET DR_DEV_PARAM = 'ROLE_DR_DEV'
SET ADMIN = CONCAT('ROLE_','DR',$env,'_ADMIN');
